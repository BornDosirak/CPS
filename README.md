...
- 1주차 발표팀임에도 불구하고 기한 내에 프로젝트를 모두 완성하였다. (센서데이터부터 시각화까지 모두 구현), 오히려 시각화의 경우 본래 계획에 없었으나 프로젝트 목적을 위해 추가해야 한다 판단하였고, 구현까지 완료했다.
- 가상의 데이터가 아닌 실제 데이터를 사용함으로써, 실제 사물 시스템의 동작에 대해 더 깊게 이해할 수 있었으며, 비슷한 다른 프로젝트로 발전시킬 때 이식률이 좋다.
- 팀원 3명 모두 관련 지식이 전무했던 아두이노, 장고, AWS를 학습하였다. 또 단순히 각각을 학습하는 것에서 그치지 않고 협업을 통해 서로를 연결하는 방법을 공부하여 하나의 프로그램으로 동작하게 만들었다.
- 모든 팀원들의 의사 소통이 원할하게 잘 이루어 졌고 task의 분배가 잘 이루어져 협업의효율성이 높았다.
