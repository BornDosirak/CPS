...
- 조건에 맞는 주제를 선정하는 방법
- 파이썬 웹 크롤링 BeautifulSoup Library 의 사용법을 익힘
- 엑셀을 통하여 데이터를 정제하는 과정에 대해 알게됨
- 아두이노 보드의 사용법을 익힘
- 센서에서 수집한 데이터를 서버로 전송하는 통신 방법을 익힘
- 파이썬 Django 서버의 구축 방법을 익힘
- Django의 Models, Views, Templates 모델을 학습하고 구현함
- Django의 Database를 관리함
- AWS 클라우드 서비스 사용방법을 익힘(Lambda 및 DynamoDB)
- AWS API gateway를 설정하면서 CORS를 배우면서 GET, PUT, POST, OPTION 메서드에 관한 이해를 높일 수 있었음
- 클라이언트와 서버가 동작하는 과정을 이해할 수 있었고, 웹 브라우저에서 AJAX 통신에 대해 깊게 이해할 수 있었음
- Git의 사용법을 익히고 프로젝트를 진행하면서 익숙해질 수 있었음
...
- CPS의 근간을 이루는 센서 데이터를 활용하고, 실습시간에 배운 크롤링을 통해 보안에 접목시켰다는 점. 강의에서 요구하는 모든 조건을 충족시킨 주제를 선정함
- 1주차 발표팀임에도 불구하고 기한 내에 프로젝트를 모두 완성함 (센서데이터부터 시각화까지 모두 구현)
- 가상의 데이터가 아닌 실제 데이터를 사용함으로써, 실제 사물 시스템의 동작에 대해 더 깊게 이해할 수 있었으며, 비슷한 다른 프로젝트로 발전시킬 때 이식률이 좋음
- 팀원 3명 모두 관련 지식이 전무했던 아두이노, 장고, AWS를 학습했다. 또 단순히 각각을 학습하는 것에서 그치지 않고 협업을 통해 서로를 연결하는 방법을 공부하여 하나의 프로그램으로 동작하게 만들었음
- 모든 팀원들의 의사 소통이 원할하게 잘 이루어 졌고, task의 분배가 잘 이루어져 협업의 효율성이 높았음
